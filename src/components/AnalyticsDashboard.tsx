...
import { getLast7DaysRange, getLastNDaysRange, getCurrentWeekRange } from '../utils/dateRange';
...
export default function AnalyticsDashboard({ 
  ...
  const p1Start = getLastNDaysRange(28).startDateStr; // 4 weeks ago
  const p2Start = getLastNDaysRange(56).startDateStr; // 8 weeks ago
  const p3Start = getLastNDaysRange(84).startDateStr; // 12 weeks ago
              ...
              <div className="space-y-5 flex-1 flex flex-col justify-between">
                <div className="space-y-5">
                  ...
                  <div className="space-y-2.5">
                    <div className="relative pl-4 border-l border-slate-800/80 space-y-3 my-1.5">
                  ...
                  {/* (4) 훈련 그룹 분포 (Push, Pull, Lower) */}
                  {(() => {
                    const { startDateStr: thirtyDaysAgoStr, endDateStr: todayStr } = getLastNDaysRange(30);

                    const last30DaysLogs = logs.filter(log => log.date >= thirtyDaysAgoStr && log.date <= todayStr);
                    ...
                    return (
                      <div className="space-y-2.5">
                        <div className="flex items-center justify-between">
                          <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">훈련 그룹 분포</h3>
                          <span className="text-[10px] font-mono text-slate-500 font-bold">웨이트 총 {strengthCount}회</span>
                        </div>

                        <div className="bg-slate-950/45 border border-slate-850/60 p-3 rounded-xl space-y-3">
                          {/* Strength Groups Row */}
                ...
                {/* 카드 하단 전체 보기 링크 */}
                {onViewAllLogs && (
                  <div className="pt-3 border-t border-slate-800/40 flex justify-end mt-auto">
                    <button 
